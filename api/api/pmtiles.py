...
async def create_pm_tiles_for_layer(layer: str, title: str, bbox: str = None):
    geojson = get_geojson_from_wfs(layer, bbox)
    with tempfile.TemporaryDirectory() as temp_dir:
        pmtiles_path = process_geojson(geojson, temp_dir)
        print(pmtiles_path)
        if pmtiles_path is not None:
            s3_path = await push_to_object_store(pmtiles_path, title)
            return s3_path
